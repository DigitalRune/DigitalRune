...
//
// This effect requires a special technique binding (C# class) to repeat the
// rendering pass several times.
//
//-----------------------------------------------------------------------------
...
VSOutput VS(VSInput input)
{
  ...
  float3 layerPositionLocal = input.Position + MaxFurLength * layer * input.Normal;
...
float4 PS(PSInput input) : COLOR
{
  ...
  if (PassIndex > 0)  // Shell 0 is always drawn completely.
  {
    ...
    float random = tex2D(JitterMapSampler, input.TexCoord / JitterMapScale);
...
technique
{
  pass
  <
    // Using a special technique binding, this pass is repeated several times.
    // The effect parameter "NumberOfLayers" defines the number of repetitions.
