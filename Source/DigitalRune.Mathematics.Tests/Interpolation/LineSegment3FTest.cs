...
namespace DigitalRune.Mathematics.Interpolation.Tests
{
  ...
  public class LineSegment3FTest
  {
    ...
    public void GetTangent()
    {
      ...
      Assert.IsTrue(Vector3F.AreNumericallyEqual(s.Point2 - s.Point1, s.GetTangent(0)));
      Assert.IsTrue(Vector3F.AreNumericallyEqual(s.Point2 - s.Point1, s.GetTangent(0.3f)));
      Assert.IsTrue(Vector3F.AreNumericallyEqual(s.Point2 - s.Point1, s.GetTangent(1)));
    }
