...
namespace DigitalRune.Ionic.Zlib
{
    ...
    internal class ZlibBaseStream : System.IO.Stream
    {
        ...
        protected int _bufferSize = ZlibConstants.WorkingBufferSizeDefault;
        ...
        private void finish()
        {
            ...
            if (_flavor == ZlibStreamFlavor.GZIP)
            {
                ...
                if (crc32_actual != crc32_expected)
                  throw new ZlibException(String.Format(CultureInfo.InvariantCulture, "Bad CRC32 in GZIP trailer. (actual({0:X8})!=expected({1:X8}))", crc32_actual, crc32_expected));

                if (isize_actual != isize_expected)
                    throw new ZlibException(String.Format(CultureInfo.InvariantCulture, "Bad size in GZIP trailer. (actual({0})!=expected({1}))", isize_actual, isize_expected));
            }
        }
