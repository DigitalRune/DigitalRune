...
namespace DigitalRune.Ionic.Zip
{
    partial class ZipEntry
    {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "defaultEncoding")]
        private static bool ReadHeader(ZipEntry ze, System.Text.Encoding defaultEncoding)
        {
            ...
            if (!ze._FileNameInArchive.EndsWith("/", StringComparison.Ordinal) && (ze._BitField & 0x0008) == 0x0008)
            {
        ...
        private int ProcessExtraFieldWinZipAes(byte[] buffer, int j, UInt16 dataSize, long posn)
        {
            if (this._CompressionMethod == 0x0063)
            {
                ...
                if (this._WinZipAesMethod != 0x01 && this._WinZipAesMethod != 0x02)
                    throw new BadReadException(String.Format(CultureInfo.InvariantCulture, "  Unexpected vendor version number (0x{0:X4}) for WinZip AES metadata at position 0x{1:X16}",
                        this._WinZipAesMethod, posn));

                Int16 vendorId = BitConverter.ToInt16(buffer, j);
                j += 2;
                if (vendorId != 0x4541)
                    throw new BadReadException(String.Format(CultureInfo.InvariantCulture, "  Unexpected vendor ID (0x{0:X4}) for WinZip AES metadata at position 0x{1:X16}", vendorId, posn));

                int keystrength = (buffer[j] == 1) ? 128 : (buffer[j] == 3) ? 256 : -1;
                if (keystrength < 0)
                    throw new BadReadException(String.Format(CultureInfo.InvariantCulture, "Invalid key strength ({0})", keystrength));
        ...
        private int ProcessExtraFieldZip64(byte[] buffer, int j, UInt16 dataSize, long posn)
        {
            ...
            if (dataSize > 28)
                throw new BadReadException(String.Format(CultureInfo.InvariantCulture,
                                                         "  Inconsistent size (0x{0:X4}) for ZIP64 extra field at position 0x{1:X16}",
                                                         dataSize, posn));
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Naming", "CA2204:Literals should be spelled correctly", MessageId = "InfoZip")]
