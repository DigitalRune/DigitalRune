...
{
  /// <summary>
  /// Writes a <see cref="SweepAndPruneSpace{T}"/> to binary format.
  /// </summary>
  /// <typeparam name="T">The type of the items in the spatial partition.</typeparam>
  [ContentTypeWriter]
  public class SweepAndPruneSpaceWriter<T> : ContentTypeWriter<SweepAndPruneSpace<T>>
  {
    /// <summary>
    ...
    {
      return typeof(SweepAndPruneSpace<T>).AssemblyQualifiedName;
    }
    ...
    {
      return typeof(SweepAndPruneSpaceReader<T>).AssemblyQualifiedName;
    }
