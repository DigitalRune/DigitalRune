...
{
  /// <summary>
  /// Writes a <see cref="DualPartition{T}"/> to binary format.
  /// </summary>
  /// <typeparam name="T">The type of the items in the spatial partition.</typeparam>
  [ContentTypeWriter]
  public class DualPartitionWriter<T> : ContentTypeWriter<DualPartition<T>>
  {
    /// <summary>
    ...
    {
      return typeof(DualPartition<T>).AssemblyQualifiedName;
    }
    ...
    {
      return typeof(DualPartitionReader<T>).AssemblyQualifiedName;
    }
