...
//-----------------------------------------------------------------------------
//
/// \file TerrainDecalLayer.fx
/// Renders a terrain decal into the terrain detail clipmap.
//
//-----------------------------------------------------------------------------
...
// Tile normal texture and size in texels
...
void VS(in float2 positionWorld : POSITION0,        // World space xz position
        inout float2 texCoordDecal : TEXCOORD0,     // Decal texture coordinates
...
void PS(in float2 texCoordDecal : TEXCOORD0,
        ...
        out float4 color2 : COLOR2)
{
  ...
  float4 diffuseMap = tex2D(DiffuseSampler, texCoordDecal);
  float4 specularMap = tex2D(SpecularSampler, texCoordDecal);
  float3 normal = GetNormalDxt5nm(NormalSampler, texCoordDecal);
  float height = tex2D(HeightSampler, texCoordDecal).x * HeightTextureScale + HeightTextureBias;
