...
{
  /// <summary>
  /// Reads a <see cref="BlendGroup"/> from binary format. 
  /// (Only available in the XNA-compatible build.)
  /// </summary>
  /// <remarks>
  /// This type is available only in the XNA-compatible build of the DigitalRune.Animation.dll.
  /// </remarks>
  public class BlendGroupReader : ContentTypeReader<BlendGroup>
  {
#if !MONOGAME
    /// <summary>
    ...
    protected override BlendGroup Read(ContentReader input, BlendGroup existingInstance)
    {
      if (existingInstance == null)
        existingInstance = new BlendGroup();
      else
      ...
      for (int i = 0; i < count; i++)
      {
        ...
        existingInstance.SetWeight(i, input.ReadSingle());
      }
