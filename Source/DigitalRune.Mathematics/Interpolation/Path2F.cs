...
namespace DigitalRune.Mathematics.Interpolation
{
  ...
  /// <remarks>
  /// <para>
  /// A <see cref="Path2F"/> is a "piecewise curve". That means, the path is defined by key points
  /// (<see cref="PathKey2F"/>) that are interpolated using spline interpolation. See 
  /// <see cref="PiecewiseCurveF{TValue,TCurveKey}"/> for more information on piecewise curves.
  /// </para>
  /// <para>
  /// <strong>Path Keys:</strong> 
  /// The path keys (<see cref="PathKey2F"/>) define the control points of the path. Each path
  ...
  public class Path2F : PiecewiseCurveF<Vector2F, PathKey2F>, IXmlSerializable
  {
    ...
    /// curve. 
    /// </remarks>
    private Vector2F GetCycleOffset(float parameter)
    {
    ...
    /// classes.
    /// </returns>
    private ICurve<float, Vector2F> GetSpline(int index)
    {
      int numberOfKeys = Count;
      Debug.Assert(index >= 0 && index < numberOfKeys);
      Debug.Assert(numberOfKeys > 1, "Path2F.GetSpline() was called for a curve without 1 full spline segment.");
      ...
      {
        var spline = StepSegment2F.Create();
      ...
      {
        var spline = LineSegment2F.Create();
        spline.Point1 = p2.Point;
        spline.Point2 = p3.Point;
        return spline;
      }
      ...
      {
        var spline = BezierSegment2F.Create();
        spline.Point1 = p2.Point;
        spline.Point2 = p3.Point;
      ...
      {
        var spline = HermiteSegment2F.Create();
        spline.Point1 = p2.Point;
        spline.Point2 = p3.Point;
      ...
      {
        var spline = BSplineSegment2F.Create();
        spline.Point1 = p1;
      ...
      {
        var spline = CatmullRomSegment2F.Create();
        spline.Point1 = p1;
        spline.Point2 = p2.Point;
        spline.Point3 = p3.Point;
        spline.Point4 = p4;
