...
{
  /// <summary>
  /// Reads a <see cref="MatrixF"/> from binary format. (Only available in the XNA-compatible
  /// build.)
  /// </summary>
  /// <remarks>
  /// This type is available only in the XNA-compatible build of the DigitalRune.Mathematics.dll.
  /// </remarks>
  public class MatrixFReader : ContentTypeReader<MatrixF>
  {
    /// <summary>
    ...
    protected override MatrixF Read(ContentReader input, MatrixF existingInstance)
    {
      ...
      {
        existingInstance = new MatrixF(numberOfRows, numberOfColumns);
      }
      else
      {
        ...
        if (existingInstance.NumberOfRows != numberOfRows || existingInstance.NumberOfColumns != numberOfColumns)
        {
          string message = String.Format(
            ...
            "Cannot load {0}x{1} MatrixF into existing {2}x{3} MatrixF.",
            numberOfRows,
            numberOfColumns,
      ...
      for (int row = 0; row < numberOfRows; row++)
        for (int column = 0; column < numberOfColumns; column++)
          existingInstance[row, column] = input.ReadSingle();
