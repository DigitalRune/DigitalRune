...
{
  /// <summary>
  /// Reads a <see cref="Path2F"/> from binary format. (Only available in the XNA-compatible build.)
  /// </summary>
  /// <remarks>
  /// This type is available only in the XNA-compatible build of the DigitalRune.Mathematics.dll.
  /// </remarks>
  public class Path2FReader : ContentTypeReader<Path2F>
  {
#if !MONOGAME
    /// <summary>
    ...
    protected override Path2F Read(ContentReader input, Path2F existingInstance)
    {
      if (existingInstance == null)
        existingInstance = new Path2F();
      else
      ...
      for (int i = 0; i < count; i++)
        existingInstance.Add(input.ReadRawObject<PathKey2F>());
