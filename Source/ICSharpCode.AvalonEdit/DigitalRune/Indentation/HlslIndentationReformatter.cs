...
{
    internal sealed class HlslIndentationReformatter
    {
        /// <summary>
        ...
        private bool _inDirective;          // Multi-line preprocessor directive  #define MACRO(x) \
        ...
        private void Step(IDocumentAccessor doc, IndentationSettings settings)
        {
            ...
            bool startInDirective = _inDirective;
            ...
            for (int i = 0; i < line.Length; i++)
            {
                if (_inDirective)
                {
                    // Check whether next line belongs to multi-line preprocessor directive.
                    // Example:
                    //
                    //   #define getrandom(min, max) \                    <-- HERE
                ...
                switch (currentChar)
                {
                    ...
                    // Check for string.
                    case '"':
                        if (!(_inDirective || _inBlockComment || inLineComment || inChar))
                        {
                            inString = !inString;
                    ...
                    // Check for character.
                    case '\'':
                        if (!(_inDirective || _inBlockComment || inLineComment || inString))
                        {
                            inChar = !inChar;
                        }
                        break;
                ...
                if (_inDirective || _inBlockComment || inLineComment || inString || inChar)
                {
            ...
            if (_block.Continuation)
            {
                // HLSL supports macros similar as in C++. Macros are usually all caps with '_'.
                // Usually, if a statement does end with a ';' then the next line is treated as 
                // a line continuation. But a macro does need to end with a ';'.
                // Example:
                // 
                //   BEGIN_CONSTANTS                                  <-- HERE
                //   float4 DiffuseColor _vs(c0)  _ps(c1)  _cb(c0);
                //   float3 SpecularColor _vs(c2)  _ps(c3)  _cb(c2);
                //   END_CONSTANTS                                    <-- HERE
                // 
                // In this case the next line is not treated as a line continuation.
                string potentialMacro = inLineComment ? doc.Text.Substring(0, _lineCommentStart) : doc.Text;
                potentialMacro = potentialMacro.Trim();
                if (IsMacro(potentialMacro))
                {
                    _block.Continuation = false;
                    _block.ResetOneLineBlock();
                }
        ...
        /// <summary>
        /// Determines whether the word is a macro.
        /// </summary>
        /// <param name="word">The word to check.</param>
        /// <returns>
        /// <see langword="true"/> if the specified word is macro; otherwise, <see langword="false"/>.
        /// </returns>
        private static bool IsMacro(string word)
        {
            if (string.IsNullOrEmpty(word))
                return false;

            // Simplified rule: Macros may only consist of upper-case character, digits 
            // and '_'. This is the commonly used conventions. (Other words are not detected.)
            ...
            {
                c = word[i];
                if (c != '_' && !char.IsUpper(c) && !char.IsDigit(c))
                    return false;
            }
