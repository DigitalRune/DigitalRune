...
namespace DigitalRune.Graphics.Interop
{
  ...
  // This class is copied from DigitalRune.Windows.

  /// <summary>
  /// The window messages. See WM_ constants in "winuser.h".
  /// </summary>
  internal static class WindowMessages
  {
    ...
    /// function returns.) The window procedure of the new window receives this message after the 
    ...
    /// procedure of the window being destroyed after the window is removed from the screen. This 
    /// message is sent first to the window being destroyed and then to the child windows (if any) 
    /// as they are destroyed. During the processing of the message, it can be assumed that all 
    /// child windows still exist.
    /// </summary>
    ...
    /// <summary>
    /// The WM_SETFOCUS message is sent when the control got the focus.
    /// </summary>
    ...
    /// <summary>
    /// An application sends the WM_SETREDRAW message to a window to allow changes in that window 
    /// to be redrawn or to prevent changes in that window from being redrawn.
    /// </summary>
    public const int WM_SETREDRAW = 0x000B;

    /// <summary>
    /// Text/Caption changed on the control. An application sends a WM_SETTEXT message to set 
    ...
    /// the text associated with a window.
    /// </summary>
    ...
    /// application calls one of the system shutdown functions. If any application returns zero, 
    /// the session is not ended. The system stops sending WM_QUERYENDSESSION messages as soon as 
    /// one application returns zero. After processing this message, the system sends the 
    /// WM_ENDSESSION message with the wParam parameter set to the results of the 
    ...
    /// It can be sent by pressing Alt+F4, Clicking the X in the upper right-hand of the program, 
    ...
    /// when a window is resized). The message is sent to prepare an invalidated portion of a 
    /// window for painting.
    /// </summary>
    ...
    /// of the WM_QUERYENDSESSION message. The WM_ENDSESSION message informs the application 
    ...
    /// change to the WIN.INI file. The SystemParametersInfo function sends this message after an 
    /// application uses the function to change a setting in WIN.INI. Note The WM_WININICHANGE 
    /// message is provided only for compatibility with earlier versions of the system. 
    /// Applications should use the WM_SETTINGCHANGE message.
    /// </summary>
    public const int WM_WININICHANGE = 0x001A;

    /// <summary>
    /// An application sends the WM_SETTINGCHANGE message to all top-level windows after making a 
    /// change to the WIN.INI file. The SystemParametersInfo function sends this message after an 
    ...
    /// <summary>
    /// The WM_SETCURSOR message is sent to a window if the mouse causes the cursor to move within 
    ...
    /// presses a mouse button. The parent window receives this message only if the child window 
    ...
    /// Windows, except in unusual circumstances explained in the MSDN documentation.
    /// </summary>
    public const int WM_PAINTICON = 0x0026;

    /// <summary>
    /// Windows NT 3.51 and earlier: The WM_ICONERASEBKGND message is sent to a minimized window 
    /// when the background of the icon must be filled before painting the icon. A window receives 
    /// this message only if a class icon is defined for the window; otherwise, WM_ERASEBKGND is 
    /// sent. This message is not sent by newer versions of Windows.
    /// </summary>
    ...
    /// list box, or menu when a visual aspect of the button, combo box, list box, or menu has 
    /// changed.
    /// </summary>
    public const int WM_DRAWITEM = 0x002B;

    /// <summary>
    /// The WM_MEASUREITEM message is sent to the owner window of a combo box, list box, list view 
    ...
    /// <summary>
    /// An application sends a WM_GETFONT message to a control to retrieve the font with which the 
    ...
    /// <summary>
    /// An application sends a WM_GETHOTKEY message to determine the hot key associated with a 
    /// window.
    /// </summary>
    ...
    /// to be dragged by the user but does not have an icon defined for its class. An application 
    /// can return a handle to an icon or cursor. The system displays this cursor or icon while 
    ...
    /// adds a new item, the system sends this message to the owner of a combo box or list box 
    ...
    /// accessible object contained in a server application. Applications never send this message 
    ...
    /// <summary>
    /// The WM_WINDOWPOSCHANGING message is sent to a window whose size, position, or place in the 
    /// Z order is about to change as a result of a call to the SetWindowPos function or another 
    ...
    /// about to enter a suspended mode. Obsolete : use POWERBROADCAST instead
    /// </summary>
    ...
    /// chooses a new input language, either with the hotkey (specified in the Keyboard control 
    /// panel application) or from the indicator on the system taskbar. An application can accept 
    /// the change by passing the message to the DefWindowProc function or reject the change (and 
    ...
    /// <summary>
    /// Sent to an application that has initiated a training card with Microsoft Windows Help. The 
    /// message informs the application when the user clicks an authorable button. An application 
    /// initiates a training card by specifying the HELP_TCARD command in a call to the WinHelp 
    /// function.
    /// </summary>
    ...
    /// WM_HELP is sent to the window associated with the menu; otherwise, WM_HELP is sent to the 
    /// window that has the keyboard focus. If no window has the keyboard focus, WM_HELP is sent 
    ...
    /// <summary>
    /// The WM_STYLECHANGING message is sent to a window when the SetWindowLong function is about 
    ...
    /// associated with a window. The system displays the large icon in the ALT+TAB dialog, and 
    /// the small icon in the window caption.
    /// </summary>
    public const int WM_GETICON = 0x007F;

    /// <summary>
    /// An application sends the WM_SETICON message to associate a new large or small icon with a 
    /// window. The system displays the large icon in the ALT+TAB dialog box, and the small icon 
    ...
    /// must be calculated. By processing this message, an application can control the content of 
    /// the window's client area when the size or position of the window changes.
    /// </summary>
    public const int WM_NCCALCSIZE = 0x0083;

    /// <summary>
    /// The WM_NCHITTEST message is sent to a window when the cursor moves, or when a mouse button 
    /// is pressed or released. If the mouse is not captured, the message is sent to the window 
    /// beneath the cursor. Otherwise, the message is sent to the window that has captured the 
    /// mouse.
    /// </summary>
    ...
    /// <summary>
    /// Non-client area activated of the window.
    /// </summary>
    ...
    /// non-client area of the window. This message is posted to the window that contains the 
    ...
    /// the cursor is within the nonclient area of a window. This message is posted to the window 
    ...
    /// window that contains the cursor. If a window has captured the mouse, this message is not 
    /// posted.
    /// </summary>
    public const int WM_NCLBUTTONDBLCLK = 0x00A3;

    /// <summary>
    /// The WM_NCRBUTTONDOWN message is posted when the user presses the right mouse button while 
    /// the cursor is within the nonclient area of a window. This message is posted to the window 
    /// that contains the cursor. If a window has captured the mouse, this message is not posted.
    /// </summary>
    ...
    /// <summary>
    /// The WM_NCMBUTTONUP message is posted when the user releases the middle mouse button while 
    /// the cursor is within the nonclient area of a window. This message is posted to the window 
    /// that contains the cursor. If a window has captured the mouse, this message is not posted.
    /// </summary>
    ...
    /// <summary>
    /// This message filters for keyboard messages. Same as WM_KEYDOWN.
    /// </summary>
    public const int WM_KEYFIRST = 0x0100;

    /// <summary>
    /// The WM_KEYUP message is posted to the window with the keyboard focus when a nonsystem key 
    /// is released. A nonsystem key is a key that is pressed when the ALT key is not pressed, or 
    ...
    /// code generated by a dead key. A dead key is a key that generates a character, such as the 
    /// umlaut (double-dot), that is combined with another character to form a composite 
    /// character. For example, the umlaut-O character (Ö) is generated by typing the dead key for 
    /// the umlaut character, and then typing the O key.
    /// </summary>
    ...
    /// window currently has the keyboard focus; in this case, the WM_SYSKEYUP message is sent to 
    /// the active window. The window that receives the message can distinguish between these two 
    ...
    /// character code of a system character key — that is, a character key that is pressed while 
    /// the ALT key is down.
    /// </summary>
    ...
    /// horizontal scroll bar. This message is also sent to the owner of a horizontal scroll bar 
    ...
    /// <summary>
    /// The WM_INITMENU message is sent when a menu is about to become active. It occurs when the 
    /// user clicks an item on the menu bar or presses a menu key. This allows the application to 
    ...
    /// <summary>
    /// The WM_MENUCHAR message is sent when a menu is active and the user presses a key that does 
    /// not correspond to any mnemonic or accelerator key. This message is sent to the window that 
    /// owns the menu.
    /// </summary>
    ...
    /// <summary>
    /// The WM_MENUDRAG message is sent to the owner of a drag-and-drop menu when the user drags a 
    /// menu item.
    /// </summary>
    ...
    /// cursor enters a menu item or moves from the center of the item to the top or bottom of the 
    /// item.
    /// </summary>
    ...
    /// <summary>
    /// An application sends the WM_CHANGEUISTATE message to indicate that the user interface (UI) 
    ...
    /// draws the message box. By responding to this message, the owner window can set the text 
    /// and background colors of the message box by using the given display device context handle.
    /// </summary>
    public const int WM_CTLCOLORMSGBOX = 0x0132;

    /// <summary>
    /// An edit control that is not read-only or disabled sends the WM_CTLCOLOREDIT message to its 
    /// parent window when the control is about to be drawn. By responding to this message, the 
    /// parent window can use the specified device context handle to set the text and background 
    ...
    /// responding to this message, the parent window can use the specified device context handle 
    ...
    /// captured, the message is posted to the window that contains the cursor. Otherwise, the 
    ...
    /// internal forwarding of the message, since DefWindowProc propagates it up the parent chain 
    /// until it finds a window that processes it.
    /// </summary>
    public const int WM_MOUSEWHEEL = 0x020A;

    /// <summary>
    /// The WM_MOUSEHWHEEL message is sent to the focus window when the mouse's horizontal scroll
    ///  wheel is tilted or rotated. The DefWindowProc function propagates the message to the 
    ...
    /// When the child window is being destroyed, the system sends the message before any 
    ...
    /// client window to maximize an MDI child window. The system resizes the child window to make 
    /// its client area fill the client window. The system places the child window's window menu 
    /// icon in the rightmost position of the frame window's menu bar, and places the child 
    /// window's restore icon in the leftmost position. The system also appends the title bar text 
    /// of the child window to that of the frame window.
    /// </summary>
    ...
    /// sizing modal loop. The window enters the moving or sizing modal loop when the user clicks 
    ...
    /// <summary>
    /// Sent to an application when a window is activated. A window receives this message through 
    ...
    /// application uses this message to control the IME window that it has created. To send this 
    /// message, the application calls the SendMessage function with the following parameters.
    /// </summary>
    ...
    /// <summary>
    /// Sent to an application to provide commands and request information. A window receives this 
    /// message through its WindowProc function.
    /// </summary>
    ...
    /// current selection, if any, in the edit control and copy the deleted text to the clipboard 
    /// in CF_TEXT format.
    /// </summary>
    ...
    /// <summary>
    /// An application sends a WM_UNDO message to an edit control to undo the last operation. When 
    /// this message is sent to an edit control, the previously deleted text is restored or the 
    ...
    /// clipboard owner must render data in the specified format and place it on the clipboard by 
    /// calling the SetClipboardData function.
    /// </summary>
    public const int WM_RENDERFORMAT = 0x0305;

    /// <summary>
    /// The WM_RENDERALLFORMATS message is sent to the clipboard owner before it is destroyed, if 
    /// the clipboard owner has delayed rendering one or more clipboard formats. For the content 
    /// of the clipboard to remain available to other applications, the clipboard owner must 
    /// render data in all the formats it is capable of generating, and place the data on the 
    ...
    /// <summary>
    /// The WM_PAINTCLIPBOARD message is sent to the clipboard owner by a clipboard viewer window 
    ...
    /// window when the clipboard contains data in the CF_OWNERDISPLAY format and an event occurs 
    /// in the clipboard viewer's vertical scroll bar. The owner should scroll the clipboard image 
    ...
    /// window. This occurs when the clipboard contains data in the CF_OWNERDISPLAY format and an 
    /// event occurs in the clipboard viewer's horizontal scroll bar. The owner should scroll the 
    ...
    /// with the keyboard focus realizes its logical palette. This message enables windows that do 
    /// not have the keyboard focus to realize their logical palettes and update their client 
    /// areas.
    /// </summary>
    ...
    /// the specified device context, most commonly in a printer device context.
    /// </summary>
